...
func TestRenderDeepBlockquotesFallBackToRaw(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "blockquote nesting too deep") {
...
func TestMaxBlockquoteMarkers(t *testing.T) {
	cases := []struct {
		in   string
		want int
	}{
		{"> > x", 2},
		{">>>", 3},
		{"- > - > x", 2},     // blockquote nested in list items
		{"1. > 2. > x", 2},   // ordered-list interposition
		{"-\t>\t-\t>\tx", 2}, // tab separators
		{": > - > x", 2},     // definition-list interposition
		{"- - - x", 0},       // pure list nesting: no '>'
		{"plain text", 0},
		{"a > b > c", 2},          // literal '>' in prose is (safely) over-counted
		{"> shallow\n> > two", 2}, // max taken across lines
	}
	for _, c := range cases {
		if got := maxBlockquoteMarkers(c.in); got != c.want {
			t.Errorf("maxBlockquoteMarkers(%q) = %d, want %d", c.in, got, c.want)
		}
	}
}
